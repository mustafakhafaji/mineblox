...
function DANDELION_GENERATION.generate(chunk_blocks: {}, surface_y: {}, SEED: number, random: Random, chunk_x: number, chunk_z: number): ({})
	...
	for x = 1, CHUNK_SIZE do
		for z = 1, CHUNK_SIZE do
			...
			for _, y in surface_y[x][z] do
				...
				local current_value = random:NextInteger(-45, 1) -- Controls frequency

				if current_chance < current_value then
					chunk_blocks[x][z][y + 1] = 'Dandelion'
				end
			end
		end
	end
